...
def main():
    ...
    p = requests.get("https://api.coingecko.com/api/v3/simple/price?ids=spell-token&vs_currencies=eth").json()
    INITIAL_PRICE = int(p['spell-token']['eth'] * 1e18)
    txparams = {"from": accounts[0]}
    if network.show_active() == 'mainnet':
        txparams.update({'required_confs': 5, 'priority_fee': '2 gwei'})
    print('SPELL price:', INITIAL_PRICE / 1e18, 'ETH')

    token = CurveTokenV5.deploy("Curve SPELL-ETH", "crvSPELLETH", txparams)
