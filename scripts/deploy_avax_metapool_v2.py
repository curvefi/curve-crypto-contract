...
def main():
    ...
    p = requests.get("https://api.coingecko.com/api/v3/simple/price?ids=bitcoin,avalanche-2&vs_currencies=usd").json()
    INITIAL_PRICES = [float(p[cur]['usd'] * 1e18) for cur in ['bitcoin', 'avalanche-2']]
    ...
    source = AvalancheCryptoSwapV2._build["source"]
